...
const Experiences = ({ educationSanity, experienceSanity }: Props) => {
  ...
  const [experiences, setExperiences] = useState(
    [...experienceSanity].reverse() as experience[]
  );
  const [educations, setEducations] = useState(
    [...educationSanity].reverse() as education[]
  );
    ...
    <SectionWrapper id="experience" className="min-h-screen">
      ...
      <div className="w-fit mx-auto mt-6 p-2 bg-white dark:bg-grey-800 rounded-md flex gap-2 items-center">
        {["Education", "Experience"].map((e, i) => (
          <button
            key={i}
            onClick={() => setShow(e)}
            className={`py-2 px-4 rounded-md transition-colors ${show === e ? "bg-blue-600 text-white" : "hover:bg-gray-100 hover:dark:bg-grey-900 text-black dark:text-white"}`}
          >
            {e}
          </button>
        ))}
      ...
      <div className="lg:container sm:mx-4 lg:mx-auto lg:w-5/6 2xl:w-3/4">
        <div className="relative wrap overflow-hidden p-4 md:py-10 md:px-0">
          ...
          {viewAll
            ? (show === "Experience" ? educations : experiences).map((e, i) => (
                // @ts-ignore
